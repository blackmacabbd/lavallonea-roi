  ...
  try {
    ...
    const result = strutture.map(s => {
      ...
      const files = db.prepare(`
        SELECT fc.id, fc.nome_file, fc.data_carico, GROUP_CONCAT(DISTINCT df.foglio) as fogli
        FROM file_caricati fc
        LEFT JOIN dati_foglio df ON df.file_id = fc.id
        WHERE fc.struttura_id = ?
        GROUP BY fc.id
        ORDER BY fc.id ASC
      `).all(s.id).map(f => ({ id: f.id, nome_file: f.nome_file, data_carico: f.data_carico, fogli: (f.fogli || '').split(',').filter(Boolean) }));
