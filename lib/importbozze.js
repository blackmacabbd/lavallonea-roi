...
const ENTITA = ['piano', 'concorrente', 'clip', 'analizzatore'];
...
function normalizzaRighe(righe) {
  ...
  for (const r of Array.isArray(righe) ? righe : []) {
    ...
    // La spunta clip va conservata insieme al nome e al prezzo: arriva dal
    // client (la casella nella revisione) e senza questo passaggio si perde
    // qui, prima ancora di raggiungere il server (task 4).
    const clip = !!(r && r.clip);

    const chiave = nome.toLowerCase();
    if (perNome.has(chiave)) duplicate++;
    perNome.set(chiave, { nome, prezzo, clip });
  }
