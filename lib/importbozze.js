...
function verificaEntita(entita) {
  if (!ENTITA.includes(entita)) {
    throw new Error(`entita non prevista: "${entita}" (attese: ${ENTITA.join(', ')})`);
  }
}
...
function normalizzaRighe(righe) {
  ...
  for (const r of Array.isArray(righe) ? righe : []) {
    ...
    // Il prezzo puo' arrivare come numero o come testo digitato dall'operatore.
    const prezzo = typeof r?.prezzo === 'number'
      ? r.prezzo
      : parseFloat(String(r?.prezzo ?? '').replace(/\s/g, '').replace(',', '.'));
    if (!nome || !Number.isFinite(prezzo) || prezzo < 0) { ignorate++; continue; }
    valide.push({ nome, prezzo });
  }
