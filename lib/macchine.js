...
function ensureSchema(db) {
  ...
  //
  // Rete di sicurezza: la premessa e' che quella tabella sia vuota. Se cosi' non
  // fosse ci troveremmo in una situazione non prevista, e distruggere righe
  // vere in silenzio a ogni avvio sarebbe il modo peggiore di scoprirlo: meglio
  // fermarsi e farlo dire da un errore, lasciando la decisione a chi guarda.
  const colonne = db.prepare(`PRAGMA table_info(macchine)`).all();
  const vecchiaForma = colonne.some(c => c.name === 'user_id');
  if (vecchiaForma) {
    const righe = db.prepare(`SELECT COUNT(*) AS c FROM macchine`).get().c;
    if (righe > 0) {
      throw new Error(
        `La tabella macchine e' nella forma precedente e contiene ${righe} righe: ` +
        `il riordino per listini non prevede una migrazione dei dati. ` +
        `Salvale o svuota la tabella prima di riavviare.`
      );
    }
    console.warn('  ! Tabella macchine nella forma precedente (vuota): ricreata per il riordino per listini');
    db.exec('DROP TABLE macchine');
  }
...
function listinoProprio(db, listinoId, userId) {
  ...
  if (!row) {
    const err = new Error('Listino non trovato');
    err.codice = 'LISTINO_NON_TROVATO';
    throw err;
  }
  return row;
}
...
function concorrenteProprio(db, concorrenteId, userId) {
  ...
  if (!row) {
    const err = new Error('Concorrente non trovato');
    err.codice = 'CONCORRENTE_NON_TROVATO';
    throw err;
  }
  return row;
}
...
function traduciDuplicato(err) {
  if (err && /UNIQUE constraint failed/.test(String(err.message))) {
    const e = new Error('Esiste gia\' una macchina con questo nome in questo listino');
    e.codice = 'MACCHINA_DUPLICATA';
    return e;
  }
  return err;
}
...
function salvaMacchina(db, dati) {
  ...
  try {
    ...
    if (!info.changes) {
      const err = new Error('Macchina non trovata');
      err.codice = 'MACCHINA_NON_TROVATA';
      throw err;
    }
    return { id: Number(id) };
  } catch (err) {
