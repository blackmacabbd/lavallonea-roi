...
function salvaMacchina(db, dati) {
  ...
  // Niente ON CONFLICT nel ramo di inserimento (a differenza di upsertMacchine):
  // l'inserimento manuale di un nome gia' presente per lo stesso proprietario
  // (utente o concorrente) deve fallire, non aggiornare in silenzio. E lo
  // stesso nome puo' scontrarsi anche nel ramo di modifica (rinominare una
  // macchina su un nome gia' usato da un'altra). Il catch e' unico per i due
  // rami, cosi' la traduzione del messaggio non si duplica: senza, l'errore
  // che arriverebbe all'operatore sarebbe quello grezzo del driver ("UNIQUE
  // constraint failed: macchine.user_id, macchine.nome" o "...,
