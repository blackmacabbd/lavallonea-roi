...
function erroreEstrazione(codice, messaggio) {
...
async function apriDocumento(buffer) {
  try {
    ...
    throw erroreEstrazione('PDF_NON_VALIDO',
      `Il file non e un PDF leggibile: ${err && err.message ? err.message : 'formato non riconosciuto'}`);
  }
}
 ...
 * @throws errore con `.codice`: PDF_NON_VALIDO, PDF_PROTETTO, PDF_SENZA_TESTO
 */
async function estraiRigheGrezze(buffer) {
  if (!buffer || !buffer.length) {
    throw erroreEstrazione('PDF_NON_VALIDO', 'File vuoto.');
  }
