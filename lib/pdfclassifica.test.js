...
// Le righe macchina non hanno mai una colonna tempi di refertazione (e la
// colonna esiste o non esiste per l'intero documento, non riga per riga):
// contarle nella statistica di togliColonnaTempi abbassa il rapporto senza
// motivo. Un capitolo di analizzatori abbastanza numeroso puo' cosi' far
// scendere il rapporto sotto soglia e sopprimere la pulizia anche per le
// righe esame che, da sole, la meriterebbero.
test('un capitolo di macchine numeroso non deve impedire la pulizia della colonna tempi sulle righe esame', () => {
  const tempiEsami = [
    'CORTISOLO/CREATININA URINARI 4 h 18,00',
    'ESAME CITOLOGICO DEL SEDIMENTO URINARIO 24 h 33,30',
    'VALUTAZIONE QUALITATIVA PROTEINURIA 7 gg 38,50',
  ...
  // Da sole, le 5 righe esame superano la soglia (5 righe, 100% >= 25%) e
  // vengono ripulite.
  const soloEsami = cls(...tempiEsami);
  assert.strictEqual(soloEsami.nomiRipuliti, 5);
  assert.strictEqual(soloEsami.incerte, 0);

  // Le stesse 5 righe, seguite da un capitolo di 30 righe macchina (nessuna
  // ha la coda tempi: le macchine non la hanno mai). Se le macchine finissero
  // nel denominatore (5 esami + 30 macchine = 35 righe con nome, contro 5 con
  // la coda: 14% < 25%), la pulizia verrebbe soppressa anche per gli esami.
  // Il risultato deve restare identico al caso "da sole".
  const macchine = [];
  for (let i = 1; i <= 30; i++) {
    macchine.push(`Analizzatore modello ${i} 1.000,00`);
  }
  const misto = cls(...tempiEsami, 'ANALIZZATORI', ...macchine);
  assert.strictEqual(misto.nomiRipuliti, 5, 'le macchine non devono abbassare il rapporto sotto soglia');
  assert.strictEqual(misto.incerte, 0, 'le righe esame vanno ripulite anche con un capitolo macchine numeroso');
  assert.strictEqual(misto.macchine, 30);
});
...
// Caso 8 (corretto): la versione precedente di questo test usava un unica
// nota che sforava insieme entrambi i limiti (126 caratteri di lunghezza E
// punto finale), per dimostrare che una nota che fallisce insieme i due
// limiti resta comunque esclusa. Ma i due controlli in tipoIntestazione sono
// in OR (t.length > SEZIONE_MAX_CARATTERI || /\.$/.test(t)): con una nota
// gia' sopra i 60 caratteri la valutazione si ferma li', e il controllo sul
// punto finale non viene mai raggiunto. Quel test quindi non provava affatto
// la protezione sul punto finale che dichiarava di provare (verificato
// rimuovendo il controllo dal punto finale dal codice: il test continuava a
// passare uguale). Come nel test sopra, servono due note isolate una per
// limite, altrimenti l'altro controllo resta comunque acceso e nasconde la
// rimozione di quello in prova: una che sfora solo la lunghezza (senza punto
// finale, e' la stessa nota di prima meno il punto), una corta (sotto i 60
// caratteri) che finisce con un punto.
test('nota in prosa oltre il limite di lunghezza, senza punto finale: non apre sezione', () => {
  const res = cls(
    'EMOCROMO COMPLETO 18,50',
  ...
  assert.ok(res.righe.every(r => r.tipo === 'esame'), 'la nota lunga non deve aprire una sezione macchine');
});

test('nota in prosa corta ma con punto finale: non apre sezione', () => {
  const res = cls(
    'EMOCROMO COMPLETO 18,50',
    'Analizzatori disponibili su richiesta in tutte le sedi.',
    'T4 TOTALE 22,90'
  );
  assert.ok(res.righe.every(r => r.tipo === 'esame'), 'la nota col punto non deve aprire una sezione macchine');
});
...
// Il caso 4 sopra ("due soli capitoli") non protegge il confine inferiore
// della soglia: usa due TESTI DIVERSI, quindi e' sotto soglia perche' nessuno
// dei due ricorre affatto, non perche' ricorra un numero di volte insufficiente.
// Per proteggere davvero MIN_PAGINE_INTESTAZIONE_CORRENTE dal basso serve lo
// STESSO testo di intestazione ripetuto su esattamente 2 pagine (2 < 3, sotto
// soglia): deve continuare a cambiare sezione a ogni occorrenza, comprese
// quelle dopo la prima. Se la soglia scendesse a 2 (2 >= 2), la ripetizione
// a pagina 2 verrebbe considerata ricorrente e, per la regola "solo la prima
// occorrenza", ignorata: la sezione macchine aperta in mezzo resterebbe
// aperta anche dopo, invece di richiudersi.
test('la stessa intestazione ripetuta su due sole pagine: sotto soglia, cambia sezione a ogni occorrenza', () => {
  const righe = [
    { pagina: 1, testo: 'ESAMI DI LABORATORIO', x: 0, y: 10, w: 300, h: 12 },
    { pagina: 1, testo: 'EMOCROMO COMPLETO 18,50', x: 0, y: 30, w: 400, h: 12 },
    { pagina: 1, testo: 'ANALIZZATORI', x: 0, y: 50, w: 300, h: 12 },
    { pagina: 1, testo: 'Analizzatore biochimico da banco 8.500,00', x: 0, y: 70, w: 400, h: 12 },
    { pagina: 2, testo: 'ESAMI DI LABORATORIO', x: 0, y: 10, w: 300, h: 12 },
    { pagina: 2, testo: 'T4 TOTALE 22,90', x: 0, y: 30, w: 400, h: 12 }
  ];
  const res = classificaRighe(righe);
  const perTipo = res.righe.filter(r => !r.scartata).map(r => [r.nome, r.tipo]);
  assert.deepStrictEqual(perTipo, [
    ...
    ['Analizzatore biochimico da banco', 'macchina'],
    ['T4 TOTALE', 'esame']
  ], 'la ripetizione a pagina 2 deve richiudere la sezione macchine: sotto soglia non e trattata come ricorrente');
});
