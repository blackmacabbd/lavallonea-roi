...
// Difetto: un'intestazione ricorrente in cima a ogni pagina (qui "LISTINO
// PREZZI - sezione N", col numero che cambia a ogni pagina) richiudeva la
// sezione macchine a ogni cambio pagina, perche' il suo testo corrisponde a
// SEZIONE_ESAMI. Su tre pagine (sopra soglia: MIN_PAGINE_INTESTAZIONE_CORRENTE
// = 3, copertura 100% >= 50%) il documento dimostra di avere un'intestazione
// corrente: le intestazioni in cima alla pagina vengono ignorate e la sezione
// macchine aperta da "ANALIZZATORI" a pagina 1 resta aperta fino alla fine.
// Le righe sono costruite a mano (non con l'helper grezza) per controllare la
// posizione verticale: l'intestazione ricorrente e' sempre in cima alla sua
// pagina (y minima), il titolo di capitolo "ANALIZZATORI" no.
test('intestazione corrente su piu pagine non richiude la sezione macchine', () => {
  const righe = [
...
// Controprova sotto soglia: con soli due titoli di capitolo su due pagine
// (2 < MIN_PAGINE_INTESTAZIONE_CORRENTE = 3) il rilevamento dell'intestazione
// corrente non scatta, quindi i titoli in cima alla pagina continuano a
// funzionare come prima e a suddividere correttamente esami e macchine: non
// deve cambiare nulla rispetto al comportamento gia' coperto dai test sopra.
test('due soli titoli di capitolo, uno per pagina: sotto soglia, continuano a dividere le sezioni', () => {
  const righe = [
