...
// Misurato prima della correzione: id 1-5, cancellate le righe 4 e 5, la
// migrazione portava seq da 5 a 3 (MAX(id) sulla tabella ricostruita, che non
// contiene piu' le righe cancellate), e il primo insert dopo riusava l'id 4 —
// il contrario del contratto dichiarato nel commento della migrazione.
test('la migrazione non abbassa il contatore autoincrement quando le righe con id piu alto sono state cancellate', () => {
  const db = new DatabaseSync(':memory:');
  db.exec(`
    CREATE TABLE analizzatori_mylav (
      id          INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  `);
  const ins = db.prepare('INSERT INTO analizzatori_mylav (user_id, nome, prezzo) VALUES (?, ?, ?)');
  for (let i = 1; i <= 5; i++) ins.run(1, `Voce ${i}`, i * 10);
  db.exec(`DELETE FROM analizzatori_mylav WHERE id IN (4, 5)`);
  assert.equal(
    db.prepare(`SELECT seq FROM sqlite_sequence WHERE name = 'analizzatori_mylav'`).get().seq, 5,
    'il contatore prima della migrazione e ancora a 5'
  );
  ...
  assert.equal(
    db.prepare(`SELECT seq FROM sqlite_sequence WHERE name = 'analizzatori_mylav'`).get().seq, 5,
    'il contatore resta a 5 dopo la migrazione, non scende a 3 (MAX(id) delle righe rimaste)'
  );
  const { id } = an.upsertAnalizzatore(db, { userId: 1, nome: 'Voce nuova', prezzo: 1 });
  assert.equal(id, 6, 'il prossimo id e 6, non il 4 gia usato e liberato');
  db.close();
});
