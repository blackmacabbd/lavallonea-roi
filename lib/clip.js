...
function upsertClip(db, dati) {
  const { userId, concorrenteId, nome, prezzoConfezione, pezzi, sconto, fonte } = dati || {};
  ...
  if (concorrenteOk == null) {
    db.prepare(`
      INSERT INTO clip (user_id, concorrente_id, nome, prezzo_confezione, pezzi, sconto, fonte)
      VALUES (?, NULL, ?, ?, ?, ?, ?)
...
function listaClip(db, userId, concorrenteId) {
  const filtra = arguments.length >= 3;
