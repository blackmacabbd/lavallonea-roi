...
// La spunta clip nasce nella revisione (client) ed e' l'unico modo che ha una
// riga per finire anche nel catalogo clip lato server (task 4): se si perde
// qui, dentro normalizzaRighe, non arriva mai alla conferma.
test('normalizzaRighe conserva la spunta clip riga per riga', () => {
  const r = bozze.normalizzaRighe([
    { nome: 'Catalyst Chem 17 CLIP 98-11003-02 12', prezzo: 448.5, clip: true },
    { nome: 'EMOCROMO COMPLETO', prezzo: 18.5, clip: false },
    { nome: 'ALTRO ESAME', prezzo: 5 }
  ]);
  assert.equal(r.valide.length, 3);
  assert.equal(r.valide[0].clip, true);
  assert.equal(r.valide[1].clip, false);
  assert.equal(r.valide[2].clip, false, 'clip assente diventa false, non undefined');
});
