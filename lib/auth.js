...
function validaPassword(pw) {
  const s = String(pw || '');
  if (s.length < 8) return { ok: false, motivo: 'La password deve avere almeno 8 caratteri', codice: 'PASSWORD_CORTA' };
  if (!/[0-9]/.test(s)) return { ok: false, motivo: 'La password deve contenere almeno un numero', codice: 'PASSWORD_SENZA_NUMERO' };
  if (!/[^A-Za-z0-9]/.test(s)) return { ok: false, motivo: 'La password deve contenere almeno un carattere speciale', codice: 'PASSWORD_SENZA_SPECIALE' };
  return { ok: true };
}
