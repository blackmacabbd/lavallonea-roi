...
function eliminaConcorrente(db, id, userId) {
  ...
  try {
    ...
    // attive (il server le tiene attive) cancellare il concorrente senza prima
    // cancellare le sue macchine viola il vincolo e fa fallire l'intera
    // transazione. Vanno cancellate qui, non lasciate orfane ne' trasformate in
    // macchine proprie (che sarebbe un dato falso): l'interfaccia promette
    // gia' all'operatore che eliminare il concorrente elimina i suoi dati.
    db.prepare(`DELETE FROM macchine WHERE concorrente_id = ?`).run(cid);
