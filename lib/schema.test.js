...
// Regressione: righe_calcolo_clip.pezzi_mylav (task 4, colonna Pezzi a
// specchio sul lato blu) deve arrivare con la stessa migrazione additiva
// delle altre. Stessa prova: nessuna riga persa, idempotente, per riga.
// Le righe salvate PRIMA di questa colonna restano con pezzi_mylav NULL, che
// nella formula del calcolatore (calcolaRigaClip/il ricalcolo server) vale
// "1", cioe' esattamente il comportamento di prima che la colonna esistesse.
test('righe_calcolo_clip: la colonna pezzi_mylav si aggiunge in modo additivo', () => {
  const db = dbVuoto();
  db.exec(CALCOLI_CLIP_DDL_PRE_LABORATORIO);
  addColIfMissing(db, 'righe_calcolo_clip', 'laboratorio', 'TEXT');
  addColIfMissing(db, 'righe_calcolo_clip', 'listino_mylav', 'TEXT');
  ...
  addColIfMissing(db, 'righe_calcolo_clip', 'pezzi_mylav', 'INTEGER');
  assert.doesNotThrow(() => addColIfMissing(db, 'righe_calcolo_clip', 'pezzi_mylav', 'INTEGER'));

  const cols = db.prepare(`PRAGMA table_info(righe_calcolo_clip)`).all().map(c => c.name);
  assert.ok(cols.includes('pezzi_mylav'));
  assert.equal(cols.filter(c => c === 'pezzi_mylav').length, 1);

  // La riga salvata prima della migrazione resta intatta, solo con
  // pezzi_mylav NULL: nessuna migrazione a ritroso, nessun dato perso.
  const vecchia = db.prepare('SELECT clip_nome, n_clip, pezzi_mylav FROM righe_calcolo_clip WHERE calcolo_id = 1').get();
  assert.equal(vecchia.clip_nome, 'Chem10 12');
  assert.equal(vecchia.n_clip, 1);
  assert.equal(vecchia.pezzi_mylav, null);

  // Una riga nuova puo' portare il proprio pezzi_mylav.
  db.prepare('INSERT INTO righe_calcolo_clip (calcolo_id, clip_nome, n_clip, pezzi_mylav) VALUES (1, ?, 1, ?)')
    .run('Chem10 12', 6);

  const righe = db.prepare('SELECT pezzi_mylav FROM righe_calcolo_clip WHERE calcolo_id = 1 ORDER BY id').all();
  assert.deepEqual(righe.map(r => r.pezzi_mylav), [null, 6]);

  db.close();
});

// Regressione sulla formula (non sullo schema): il costo per unita' del
// calcolatore clip/Mylav non deve piu' tornare "null" quando i pezzi sono
// mancanti/zero — deciso dal cliente due volte (task 1a/4): il calcolatore
// assume 1, il catalogo (una tabella diversa, non toccata qui) resta invece
// vuoto quando i pezzi non si conoscono. Replica la formula di
// calcolaRigaClip/costoAnalizzatore (public/app.js) e del ricalcolo server
// (POST /api/calcolo-clip/salva): le tre copie devono restare identiche, e
// un test qui e' l'unico modo di accorgersi se una delle tre si scosta.
function costoPerUnita(prezzo, pezzi, sconto) {
  const pezziGrezzi = parseFloat(pezzi) || 0;
  const p = parseFloat(prezzo) || 0;
  const s = parseFloat(sconto) || 0;
  const divisore = pezziGrezzi > 0 ? pezziGrezzi : 1;
  return parseFloat((p / divisore * (1 - s / 100)).toFixed(2));
}

test('costo per unita clip/Mylav: pezzi mancanti o zero contano come 1, mai null', () => {
  assert.equal(costoPerUnita(120, null, 0), 120);
  assert.equal(costoPerUnita(120, 0, 0), 120);
  assert.equal(costoPerUnita(120, '', 0), 120);
  assert.equal(costoPerUnita(120, 12, 0), 10);
  assert.equal(costoPerUnita(100, 4, 10), 22.5);
});
