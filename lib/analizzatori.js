...
function ensureSchema(db) {
  ...
  try {
    if (daRicostruire) {
      // Il contatore AUTOINCREMENT va letto ORA, prima del DROP TABLE qui
      // sotto che si porta via anche la riga di sqlite_sequence della tabella
      // vecchia. MAX(id) sulla tabella ricostruita NON e' un sostituto: se nel
      // frattempo delle righe con gli id piu' alti sono state cancellate (es.
      // id 1-5, cancellate le righe 4 e 5), MAX(id) sulla tabella ricostruita
      // torna 3, piu' basso del contatore vero (5) — impostare seq=3 e'
      // equivalente a non avere nessuna riga in sqlite_sequence, e il prossimo
      // insert riassegnerebbe l'id 4 gia' usato e liberato. Misurato: senza
      // questa lettura anticipata, seq passava da 5 a 3 e il primo import
      // successivo riusava l'id 4, contraddicendo il contratto dichiarato qui
      // sotto (gli id non cambiano ne' si riusano).
      const vecchioSeq = db.prepare(
        `SELECT seq FROM sqlite_sequence WHERE name = 'analizzatori_mylav'`
      ).get();
      db.exec(`
      ...
      `);
      // Si ripristina il contatore al piu' alto fra quello vecchio (letto
      // sopra, prima di sparire col DROP TABLE) e il MAX(id) della tabella
      // appena ricostruita: il primo copre il caso di righe cancellate coi id
      // piu' alti, il secondo resta una rete di sicurezza se per qualche
      // motivo la tabella vecchia non aveva ancora una riga in
      // sqlite_sequence (mai vista finora, ma un MAX(id) reale non deve mai
      // essere ignorato). Nessuna riga se la tabella e' vuota E non c'era gia'
      // un contatore: una tabella mai popolata non ha bisogno di una riga in
      // sqlite_sequence.
      db.exec(`DELETE FROM sqlite_sequence WHERE name = 'analizzatori_mylav'`);
      const nuovoMax = db.prepare(`SELECT MAX(id) AS m FROM analizzatori_mylav`).get().m;
      const seqDaRipristinare = Math.max(vecchioSeq ? vecchioSeq.seq : 0, nuovoMax || 0);
      if (seqDaRipristinare > 0) {
        db.prepare(`INSERT INTO sqlite_sequence (name, seq) VALUES ('analizzatori_mylav', ?)`)
          .run(seqDaRipristinare);
      }
    }
