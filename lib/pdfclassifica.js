...
// dura piu' pagine: il testo cambia da pagina a pagina (cambia il numero),
// quindi confrontare il testo non basta, va usata la posizione. Tolleranza in
// punti: piccole differenze di layout fra pagine (interlinea, arrotondamenti)
// spostano di pochi punti la riga piu' in alto, senza che sia una riga diversa.
const TOLLERANZA_CIMA_PAGINA = 5;
// Un titolo di capitolo vero compare una sola volta, in cima alla sua prima
// pagina: un listino con 5 capitoli su 117 pagine ha quindi titoli in cima
// solo al 4% delle pagine. Un'intestazione corrente invece e' in cima a
// (quasi) tutte le pagine. La quota da sola non basta: su un documento breve
// (2-3 pagine) anche pochi titoli veri coprono gia' il 50-100%, quindi serve
// anche un minimo assoluto di pagine coinvolte, non solo la percentuale.
const MIN_PAGINE_INTESTAZIONE_CORRENTE = 3;
const QUOTA_PAGINE_INTESTAZIONE_CORRENTE = 0.5;
...
// Y minima fra le righe di ogni pagina: e' la cima di quella pagina. Serve
// sia per rilevare un'intestazione corrente sia per ignorarla in assegnaTipo.
function cimePagina(righe) {
  const cime = new Map();
  for (const r of righe) {
    const attuale = cime.get(r.pagina);
    if (attuale === undefined || r.y < attuale) cime.set(r.pagina, r.y);
  }
  return cime;
}

// Vero se la riga e' entro la tolleranza dalla cima della sua pagina.
function inCimaAllaPagina(riga, cime) {
  const cima = cime.get(riga.pagina);
  return cima !== undefined && (riga.y - cima) <= TOLLERANZA_CIMA_PAGINA;
}

/**
 * Prova documentale (come togliColonnaTempi): un'intestazione ricorrente si
 * riconosce dal documento intero, non dalla singola riga. Vero se una riga
 * candidata a intestazione di sezione (tipoIntestazione la riconosce) compare
 * in cima a una quota consistente delle pagine: a quel punto e' un titolo
 * corrente di pagina, non un titolo di capitolo, anche se il suo testo cambia
 * a ogni pagina ("sezione 1", "sezione 2", ...) e quindi non e' riconoscibile
 * confrontando il testo.
 */
function haIntestazioneCorrente(righe, cime) {
  const pagine = new Set(righe.map(r => r.pagina));
  if (pagine.size === 0) return false;

  const pagineConCandidataInCima = new Set();
  for (const r of righe) {
    if (tipoIntestazione(r) && inCimaAllaPagina(r, cime)) pagineConCandidataInCima.add(r.pagina);
  }
  return pagineConCandidataInCima.size >= MIN_PAGINE_INTESTAZIONE_CORRENTE
    && pagineConCandidataInCima.size >= pagine.size * QUOTA_PAGINE_INTESTAZIONE_CORRENTE;
}
 ...
 *
 * Se il documento dimostra di avere un'intestazione corrente (vedi
 * haIntestazioneCorrente), le intestazioni in cima alla pagina vengono
 * ignorate: altrimenti un titolo ricorrente ("LISTINO PREZZI - sezione N")
 * richiuderebbe a ogni pagina una sezione macchine che dura piu' pagine.
 */
function assegnaTipo(righe) {
  const cime = cimePagina(righe);
  const ignoraCima = haIntestazioneCorrente(righe, cime);

  let corrente = 'esame';
  return righe.map(r => {
    const sezione = tipoIntestazione(r);
    if (sezione && !(ignoraCima && inCimaAllaPagina(r, cime))) corrente = sezione;
