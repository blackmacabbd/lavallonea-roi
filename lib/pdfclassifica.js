...
// Un'intestazione corrente (titolo ricorrente in cima a ogni pagina, es.
// "LISTINO PREZZI - sezione 3") non deve richiudere una sezione macchine che
// dura piu' pagine. La discriminante non e' la posizione: anche un titolo di
// capitolo vero sta in cima alla sua pagina, quindi guardare dove si trova la
// riga non distingue i due casi (un documento con 4 capitoli veri, uno per
// pagina, ha 4 titoli tutti "in cima", esattamente come un'intestazione
// ricorrente). La discriminante e' la ricorrenza del testo, a meno delle
// cifre: un'intestazione corrente ripete lo stesso testo normalizzato su
// molte pagine diverse ("sezione 1", "sezione 2", ... sono lo stesso testo
// senza le cifre), mentre due titoli di capitolo sono testi diversi. Sotto
// soglia il segnale non e' affidabile: un documento breve puo' avere per
// caso 2 capitoli veri consecutivi (vedi il test "due soli capitoli"); da 3
// pagine distinte in su, che siano 3 capitoli diversi col medesimo testo
// normalizzato e' un caso che nei listini reali non si presenta.
const MIN_PAGINE_INTESTAZIONE_CORRENTE = 3;
...
function togliColonnaTempi(righe) {
  // La statistica va calcolata solo sulle righe di tipo esame: la colonna dei
  // tempi di refertazione e' una caratteristica della tabella degli esami, e
  // le righe macchina non la hanno mai. Contarle (al numeratore o al
  // denominatore) falsa la misura: un capitolo di analizzatori abbastanza
  // grande farebbe scendere il rapporto sotto soglia e sopprimerebbe la
  // pulizia anche per le righe esame che, da sole, la meriterebbero. Per lo
  // stesso motivo le macchine non vanno mai ripulite (sotto, nella map).
  const conNome = righe.filter(r => !r.scartata && r.nome && r.tipo === 'esame');
  ...
  const pulite = righe.map(r => {
    if (r.scartata || !r.nome || r.tipo !== 'esame' || !CODA_COLONNA_TEMPI.test(r.nome)) return r;
...
/**
 * Normalizza il testo di una candidata intestazione per confrontare
 * occorrenze su pagine diverse: minuscolo, via le cifre (sono cio' che
 * cambia da pagina a pagina in "sezione 1", "sezione 2", ...), via la
 * punteggiatura, spazi compattati. Due intestazioni correnti della stessa
 * serie diventano cosi' lo stesso testo; due titoli di capitolo diversi
 * restano testi diversi anche dopo la normalizzazione.
 *
 * @param {string} testo
 * @returns {string}
 */
 ...
 *
 * Prima passata (prova documentale, come togliColonnaTempi): per ogni testo
 * di intestazione normalizzato (vedi normalizzaIntestazione), su quante
 * pagine distinte compare. Serve a distinguere un'intestazione corrente
 * (ricorre sullo stesso testo su piu' pagine) da un titolo di capitolo
 * (compare una volta sola, con un testo che non si ripete).
 *
 * Seconda passata: un'intestazione il cui testo normalizzato ricorre su
 * almeno MIN_PAGINE_INTESTAZIONE_CORRENTE pagine distinte cambia la sezione
 * solo alla prima occorrenza; le occorrenze successive vengono ignorate,
 * perche' la sezione e' gia' quella giusta (se il titolo di un capitolo
 * macchine e' ripetuto in cima a tutte le sue pagine, la prima occorrenza
 * deve comunque aprire la sezione: le ripetizioni successive sono innocue).
 * Un'intestazione che ricorre su meno pagine di cosi' cambia sempre la
 * sezione, a ogni occorrenza.
 */
function assegnaTipo(righe) {
  const paginePerTesto = new Map();
  for (const r of righe) {
    if (!tipoIntestazione(r)) continue;
    const chiave = normalizzaIntestazione(r.testo);
    if (!paginePerTesto.has(chiave)) paginePerTesto.set(chiave, new Set());
    paginePerTesto.get(chiave).add(r.pagina);
  }
  ...
  return righe.map(r => {
    ...
    if (sezione) {
      const chiave = normalizzaIntestazione(r.testo);
      const ricorrente = paginePerTesto.get(chiave).size >= MIN_PAGINE_INTESTAZIONE_CORRENTE;
      if (!ricorrente || !applicate.has(chiave)) {
        corrente = sezione;
        applicate.add(chiave);
      }
    }
 ...
 * @returns {{
 *   righe: Array<Object>,      // ogni riga grezza + nome, prezzo, confidenza, tabellare, scartata, motivo, indice, tipo
 *   totaliTabellari: number,   // righe che contengono un importo
 *   classificate: number,      // righe accettate come esame
 *   alta: number,
 *   incerte: number,
 *   scartate: number,
 *   esami: number,             // righe non scartate di tipo 'esame'
 *   macchine: number,          // righe non scartate di tipo 'macchina'
