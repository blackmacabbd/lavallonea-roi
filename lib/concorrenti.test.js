...
function dbConCatalogo() {
  ...
  ensureSchema(db);
  // Come in server.js: le due tabelle coesistono sempre nello stesso db, ed
  // eliminaConcorrente ora cancella anche da macchine, quindi la tabella deve
  // esistere anche nei test che usano questo fixture (es. eliminaConcorrente).
...
// macchine.concorrente_id REFERENCES concorrenti(id): con le chiavi esterne
// attive (come le tiene il server) eliminare il concorrente senza eliminare
// prima le sue macchine viola il vincolo e fa fallire la transazione. Qui
// attiviamo la PRAGMA esplicitamente invece di affidarci al default della
// connessione, cosi' il test resta valido anche se quel default cambiasse.
test('eliminaConcorrente elimina anche le sue macchine (FK attive): niente righe orfane in nessuna delle due tabelle', () => {
  const db = dbVuoto();
  db.exec('PRAGMA foreign_keys = ON');
  ensureSchema(db);
  macchine.ensureSchema(db);
  const { concorrenteId } = upsertConcorrente(db, 'IDEXX', [{ nome_originale: 'EMOCROMO', prezzo: 18.5, sconto: null }], 1);
  macchine.upsertMacchine(db, { userId: 1, concorrenteId, righe: [{ nome: 'Analizzatore', prezzo: 8500 }] });

  assert.equal(eliminaConcorrente(db, concorrenteId, 1), true);
  assert.equal(db.prepare('SELECT COUNT(*) c FROM concorrenti WHERE id = ?').get(concorrenteId).c, 0);
  assert.equal(db.prepare('SELECT COUNT(*) c FROM esami_concorrente WHERE concorrente_id = ?').get(concorrenteId).c, 0);
  assert.equal(db.prepare('SELECT COUNT(*) c FROM macchine WHERE concorrente_id = ?').get(concorrenteId).c, 0);
  db.close();
});
