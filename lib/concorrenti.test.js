...
// I listini di macchine sono legati al concorrente da listini_macchine.concorrente_id
// REFERENCES concorrenti(id): con le chiavi esterne attive (come le tiene il
// server) eliminare il concorrente senza eliminare prima i suoi listini viola
// il vincolo e fa fallire la transazione. Qui attiviamo la PRAGMA esplicitamente
// invece di affidarci al default della connessione, cosi' il test resta valido
// anche se quel default cambiasse.
test('eliminare un concorrente porta via anche i suoi listini di macchine', () => {
  const db = dbVuoto();
  db.exec('PRAGMA foreign_keys = ON');
  ensureSchema(db);
  macchine.ensureSchema(db);
  const { concorrenteId } = upsertConcorrente(db, 'IDEXX', [{ nome_originale: 'EMOCROMO', prezzo: 18.5 }], 1);
