...
function upsertFromJson(db, data) {
  const getEsame = db.prepare(`SELECT id FROM esami_riferimento WHERE nome = ? AND user_id IS NULL`);
  const insEsame = db.prepare(`INSERT INTO esami_riferimento (nome, prezzo_base, user_id) VALUES (?, ?, NULL)`);
  const updEsame = db.prepare(`UPDATE esami_riferimento SET prezzo_base = ? WHERE id = ?`);
  const getPiano = db.prepare(`SELECT id FROM piani_sconto WHERE nome = ? AND user_id IS NULL`);
  const insPiano = db.prepare(`INSERT INTO piani_sconto (nome, categoria, anno, ordine, attivo, user_id) VALUES (?, ?, ?, ?, 1, NULL)`);
  ...
  try {
    for (const [nomeRaw, prezzo] of Object.entries(data.exams_base_price || {})) {
      ...
      const row = getEsame.get(nome);
      if (row) updEsame.run(prezzo, row.id); else insEsame.run(nome, prezzo);
    }
    const ordine = data.plan_order || Object.keys(data.plans || {});
    ordine.forEach((nomePiano, idx) => {
      const esistente = getPiano.get(nomePiano);
      if (esistente) updPiano.run(categoriaDiPiano(nomePiano), annoDiPiano(nomePiano), idx, esistente.id);
      else insPiano.run(nomePiano, categoriaDiPiano(nomePiano), annoDiPiano(nomePiano), idx);
      const pianoId = getPiano.get(nomePiano).id;
      for (const [nomeEsameRaw, prezzo] of Object.entries((data.plans || {})[nomePiano] || {})) {
        const esameRow = getEsame.get(norm(nomeEsameRaw));
...
function pianoMigliorePerEsame(db, esameNomeRaw, userId) {
  ...
  const rows = db.prepare(`
    ...
    WHERE p.attivo = 1
  `).all(nome, nome, userId != null ? userId : -1);
...
function pianoMiglioreTotale(db, esami, userId) {
  ...
  const piani = db.prepare(`SELECT id, nome FROM piani_sconto WHERE attivo = 1 ORDER BY ordine`).all();
  let best = null;
...
module.exports = {
  norm, ensureSchema, copiaCatalogoPerUtente, categoriaDiPiano, annoDiPiano, CATEGORIE,
