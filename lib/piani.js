...
function totalePiano(db, pianoId, esami) {
  let totale = 0, nEsami = 0, nSaltati = 0;
  for (const e of (esami || [])) {
    const r = resolvePrezzo(db, pianoId, e.nome);
...
function pianoMiglioreTotale(db, esami) {
  ...
  for (const p of piani) {
    const t = totalePiano(db, p.id, lista);
...
function pianiClassifica(db, esami) {
  const lista = (esami || []).filter(e => e && e.nome && String(e.nome).trim());
