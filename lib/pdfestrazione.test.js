...
test('buffer non PDF: errore PDF_NON_VALIDO, con la causa tecnica in dettaglio', async () => {
  await assert.rejects(
    () => estraiRigheGrezze(Buffer.from('questo non e un pdf')),
    err => {
      assert.strictEqual(err.codice, 'PDF_NON_VALIDO');
      // Il dettaglio viaggia a parte perche' il client mostra la frase tradotta
      // e ci sostituisce dentro la causa: senza, l'operatore leggerebbe che il
      // file non si apre e non saprebbe perche'.
      assert.ok(err.dettaglio, 'il dettaglio tecnico deve esserci');
      // Frase esatta, non solo il codice: il difetto che questo test difende era
      // proprio una frase italiana cambiata mentre il codice restava giusto.
      assert.strictEqual(err.message, 'Il file non e un PDF leggibile: ' + err.dettaglio);
      return true;
    }
  );
});

// Percorso separato da PDF_NON_VALIDO: un file vuoto non e' un PDF malformato,
// e il motivo che si mostra all'operatore e' diverso.
test('buffer vuoto: errore PDF_VUOTO', async () => {
  for (const vuoto of [Buffer.alloc(0), null, undefined]) {
    await assert.rejects(
      () => estraiRigheGrezze(vuoto),
      err => {
        assert.strictEqual(err.codice, 'PDF_VUOTO');
        assert.strictEqual(err.message, 'File vuoto.');
        return true;
      }
    );
  }
