...
async function sendViaSendGrid({ to, subject, html }) {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      console.error('[mailer] SendGrid error', resp.status, body);
      return { sent: false, mode: 'sendgrid', error: `HTTP ${resp.status}: ${body}` };
    }
    return { sent: true, mode: 'sendgrid' };
  } catch (err) {
    console.error('[mailer] SendGrid invio fallito', err.message);
    return { sent: false, mode: 'sendgrid', error: err.message };
  }
}
